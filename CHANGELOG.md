## [1.5.6] — 2026-09-26 — provenance you can check, and a silent drop that is no longer silent

Two findings, one release. `PROTOCOL_VERSION` stays 1.5.0, `SCHEMA_VERSION` 1.0.0; the wire contract, the signed shape and every conformance vector are untouched.

- **THE ENVELOPE'S ONLY PROVENANCE FIELD WAS THE ONE FIELD A READER COULD NOT CHECK.** `MANIFEST.json` carried `source_commit`, a commit of the **build** repository. The public repository is a squash export, so that object is not in it: an independent arm resolved the published v1.5.1's commit and got `bad object`. 1.5.2 answered the same finding in **prose** ("the seal rests on the hashes"), which reads like a check without being one — nothing in the envelope was re-derived from the commit's place, and nothing forbade the next unverifiable field from being added the same way.
  Fix: the manifest carries a **content digest** — sha256 over one `"<sha256>  <path>"` line per sealed file, sorted by path, which is reproducible from any fresh clone or unpacked archive with `sha256sum <the sealed paths> | LC_ALL=C sort -k2 | sha256sum` — and chapter 1 **re-derives it from the shipped bytes** rather than reading it back. Every manifest field is now **classified**: `verifiable_in_public` (each listed name must have a real check behind it in `evidence.PUBLIC_CHECKS`, and that check runs — the version, for instance, is bound to the shipped `product/version.py`), or `unverifiable_in_public` with a mandatory `reason` and a `covered_instead_by` that either names a field that IS re-derived or says, out loud, `nothing — stated as an unchecked claim`. A field in **neither** class fails the chapter, so a check cannot go missing quietly and a field cannot be dropped instead of explained. `source_commit` stays in the manifest — dropping it would hide the gap, not close it — marked unverifiable-in-public and pointing at the content digest; the verifier prints that line every run. The release gate checks the same thing before publishing, so an unanchored envelope cannot reach a buyer again.
- **DELIVERY IS NOT PROCESSING, and only delivery was written down.** `recv --mark` and the SSH tier's `mark_delivered` prove the transport handed a message over. Neither says the receiving side did anything with it, so a handler that returned early, filtered the message away or swallowed its own exception left a bus that looked perfectly healthy — the row read, the cursor moved, `reconcile` empty — and the message gone. **This was the hole: the failure was invisible to every check the bus had.**
  Fix: the receiving side acknowledges PROCESSING separately, into the same hash-chained `cursor_audit` (`processed` / `processed_refused` rows — free-text `op`, so no schema change, as with `delivered_id` and `sig`), and `agent_bus.py silent-drops --agent X` compares the ids the chain records as **delivered** against the ids that carry a processing ack. The difference is a delivery nobody acted on, and it exits non-zero so a cron job or a CI step sees the silence without anyone going to look for it. An ack may cover **only** an id the chain records as delivered to that agent — otherwise a receiver buys silence by claiming everything — and a refused claim enters the chain in its own `processed_refused` row rather than being merely returned. `--grace N` ignores deliveries younger than N seconds (in flight, not dropped); the default is 0, because a detector that hides findings by default is not a detector. This is the question NEXT TO `reconcile`, not the same one: `reconcile` lists what the cursor stepped over **without delivering**, `silent-drops` lists what was delivered **into a void**. The coverage floor gains a tenth claim: remove the comparison and six tests decide against it.
- **A BROKEN CRYPTO BACKEND TOOK DOWN MODULES THAT ARE SUPPOSED TO DEGRADE.** The optional `cryptography` imports were guarded with `except Exception`, but an **installed** library whose native backend cannot load raises from outside the `Exception` tree (pyo3's `PanicException` derives from `BaseException`), so `import agent_bus` — and `bus_notary`, `sds_envelope`, `bus_relay` — died instead of falling back to the already-supported "no crypto" state. Measured on a machine whose `cryptography` could not load `_cffi_backend`: the envelope reported chapter 2 as **FAIL**, "the notary chain did not bite", about a chapter that was never measured. Unusable is the same state as absent, and absent was always handled (fail-closed, chapter reports SKIP). Same guard in all four modules.
- **AN UNMEASURED CHAPTER IS NOT A MEASURED FAILURE.** The coverage floor runs the collector the product ships with; without `pytest` the runner never reached a test body, produced zero per-test outcomes, and the chapter reported nine claims as **FAIL ... (0 red)** in one second — a measurement that did not happen, stated as one that did. The floor now reports `NOT MEASURED` for a claim whose baseline produced no outcome at all, and chapter 3 says `NOT MEASURED` when the shipped runner is absent. It is still not a pass: `make_evidence` and the release gate both refuse to cut a release on it, and the verdict line lists it among the unmeasured chapters.
- **The seal:** re-sealed as a NEW version — 1.5.5 is not resealed in place, so each version names exactly one byte set.
